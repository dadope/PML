...
class DDPM(BaseSampler):
	...
	def p_sample_loop(self, model, x_start, measurement_cond_fn=None, record=False, save_root=None, **kwargs):
		...
		"""
		x = x_start
		progress_dir = os.path.join(save_root, "progress")
		...
		for t in range(self.steps):
			...
			x = model(x, t) + noise / (t**(0.9) + 1)  # Simplified sampling step

			if record:
				save_path = os.path.join(progress_dir, f"step_{t:03d}.png")
				plt.imsave(save_path, clear_color(x))  # No need to convert x to numpy
