...
def main():
    ...
    logger.info(f"Model Architecture:\n{model}")  # Log model architecture

    # Load checkpoint and handle potential structure issues
    checkpoint = torch.load(args.model_checkpoint, map_location=device)
    if "state_dict" in checkpoint:
        checkpoint = checkpoint["state_dict"]  # Extract state_dict if wrapped

    # Debugging: Inspect the shapes of checkpoint parameters
    ...
    model.eval()
    logger.info("Trained model loaded and set to evaluation mode.")
