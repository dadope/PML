...
def extract_and_expand(array, t, target):
    """
    Extract values from a 1-D numpy array for specific timesteps and expand them to match the shape of the target tensor.
    ...
    """
    array = torch.from_numpy(array).to(target.device)[t].float()
