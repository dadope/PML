...
def train_model(model, diffusion, train_loader, val_loader, epochs, optimizer, scheduler, criterion, save_dir):
	...
	for epoch in range(epochs):
		...
		logger.info(f"Epoch {epoch + 1}/{epochs}, Loss: {avg_epoch_loss:.4f}, Val Loss: {avg_val_loss:.4f}")

		# Save checkpoint
		os.makedirs(save_dir, exist_ok=True)
		torch.save(
			model.state_dict(),  # Save only the model state_dict
...
def perform_inference(configs, task_config, model_checkpoint, save_dir):
	...
	sampler_args = {k: v for k, v in diffusion_config.items() if k not in {"betas", "rescale_timesteps", "sampler_name"}}
